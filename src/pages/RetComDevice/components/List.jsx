...
export default function List({
  ...
  const [contentSizes, setContentSizes] = useState(() => {
    try {
      const saved = localStorage.getItem(RETCOM_NODE_CONTENT_SIZE_KEY);
  ...
  const handleSetContentSize = (path, size) => {
    setContentSizes(prev => {
      ...
      try {
        localStorage.setItem(RETCOM_NODE_CONTENT_SIZE_KEY, JSON.stringify(updated));
      } catch (e) {
