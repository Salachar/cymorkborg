  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      setDiscoveredPasswords(prev => {
        ...
        try { localStorage.setItem(PASSWORD_STORAGE_KEY, JSON.stringify(next)); } catch {}
        return next;
      });
  ...
  const handleEvent = (event) => {
    switch (event.type) {
      ...
      case 'indent_change':
        ...
        try { localStorage.setItem(LIST_INDENT_KEY, String(event.value)); } catch {}
        break;
      ...
      case 'toggle_bookmark':
        setBookmarks(prev => {
          ...
          try { localStorage.setItem(BOOKMARKS_STORAGE_KEY, JSON.stringify(next)); } catch {}
          return next;
        });
        break;
