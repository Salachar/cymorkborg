...
import './maintenanceAccess.css';

const VARIANT_CONFIG = {
  default: {
    stampLabel: null,
    footerText: '⚠ Authorized personnel only - All actions logged',
  },
  internal: {
    stampLabel: 'INTERNAL ACCESS',
    footerText: '// session active — actions logged to internal audit trail',
  },
  backdoor: {
    stampLabel: '◈ RCD-7_INJECT.SYS',
    footerText: '⚠ unauthorized access — traffic may be monitored',
  },
};
...
export default function MaintenanceAccess({
  ...
  return (
    <div className={`ma-root ma-variant-${variant}`}>
      <div className="ma-shell">

        {/* Header */}
        <div className="ma-header">
          {config.stampLabel && (
            <span className="ma-header-stamp">{config.stampLabel}</span>
          )}
          <span className="ma-header-title">{title}</span>
        </div>
        ...
        <div className="ma-body">
          ...
          <div className="ma-section">
            <div className="ma-section-header">
              <span className="ma-section-label">SYSTEM INFORMATION</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                ...
                />
                <span style={{ fontSize: '0.7rem', fontWeight: 'bold', color: statusColor, fontFamily: 'monospace' }}>
                  {systemStatus}
                </span>
              </div>
            </div>
          ...
          {notes.length > 0 && (
            <div className="ma-section">
              <span className="ma-section-label" style={{ display: 'block', marginBottom: '0.5rem' }}>
                NOTES
              </span>
              {notes.map((note, i) => (
                <div key={i} className="ma-note">
                  <span className="ma-note-bullet">•</span>
                  <span className="ma-note-text">{note}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {Boolean(children) && (
            <div className="ma-children">{children}</div>
          )}

          {/* Footer */}
          <div className="ma-footer">{config.footerText}</div>
        </div>
      </div>
    </div>
  );
